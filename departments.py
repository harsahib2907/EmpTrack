...
class Department_class:
    def __init__(self,window):
        ...
        self.head=Label(self.window,text="Departments",background=mycolor1,foreground=mycolor4,font=myfont2,relief="raised",borderwidth=10)
        #--------------------Labels----------------------
        self.L1=Label(self.window,text="Department Id",background=mycolor2,foreground=mycolor4,font=myfont1)
        ...
        #----------------------Table------------------------
        self.mytable=Treeview(self.window,columns=["c1","c2","c3"])
        self.mytable.heading("c1",text="Department Id")
        self.mytable.heading("c2",text="Name")
        self.mytable.heading("c3",text="Abbreviation")
        self.mytable["show"]="headings"
        self.mytable.column("c1", width=260,anchor="center")
        self.mytable.column("c2", width=260, anchor="center")
        self.mytable.column("c3", width=260, anchor="center")
    ...
    def SaveData(self):
        if self.Validation()==False:
            return
        #department_id	department_name	department_abb
        try:
            qry="insert into department values(%s,%s,%s)"
            rowcount=self.curr.execute(qry , (self.e1.get(),self.e2.get(),self.e3.get().upper()))
            self.conn.commit()
            if rowcount==1:
    ...
    def UpdateData(self):
        if self.Validation()==False:
            return
        try:
            qry="update department set department_name=%s,department_abb=%s where department_id=%s"
            rowcount=self.curr.execute(qry , (self.e2.get(),self.e3.get(),self.e1.get()))
            self.conn.commit()
            if rowcount==1:
    ...
    def DeleteData(self):
        ...
        if ans=="yes":
            try:
                qry="delete from department where department_id=%s"
    ...
    def Search_Data(self):
            try:
                qry="select * from department where department_name like %s"
