...
class Attendence_class:
    ...
    def __init__(self,window):
        ...
        self.head=Label(self.window,text="Attendance Tracker",background=mycolor1,foreground=mycolor4,font=myfont2,relief="raised",borderwidth=10)
        #--------------------Labels----------------------
        ...
        self.L4 = Label(self.window, text="Status", background=mycolor2, foreground=mycolor4,font=myfont1)
        ...
        self.v4 = StringVar()
        self.c4 = Combobox(self.window,textvariable=self.v4,values=["Persent","Absent","On Leave"], background=mycolor3, foreground=mycolor1, font=myfont1,state="readonly")
        #------------------Buttons-------------------
        self.b1=Button(self.window,text="\U00002713Mark \nAttendance",background=mycolor3,foreground=mycolor2,font=("PT Serif",13,"bold"),command=self.SaveData)
    ...
    def SaveData(self):
        ...
        try:
            ...
            rowcount=self.curr.execute(qry , (self.e1.get().strip(),self.e2.get(),self.e3.get(),self.v4.get()))
            self.conn.commit()
            if rowcount==1:
    ...
    def UpdateData(self):
        ...
        if self.Validation()==False:
            return
        try:
            qry="update attendance set name=%s,date=%s,status=%s where emp_id=%s"
            rowcount=self.curr.execute(qry , (self.e2.get(),self.e3.get(),self.v4.get(),self.e1.get().strip()))
            self.conn.commit()
            if rowcount==1:
    ...
    def DeleteData(self):
        ...
        if ans=="yes":
            try:
                qry="delete from attendance where emp_id=%s"
    ...
    def Fetchdata(self,pkcol=None):
        if pkcol==None:
            ...
            empid=pkcol
        try:
            qry = "select * from  attendance  where emp_id=%s"
    ...
    def Search_Data(self):
            try:
                qry="select * from attendance where date like %s"
    ...
    def Validation(self):
        if len(self.e3.get())<1 :
            ...
            return False
        elif self.v4.get() == "Mark Attendance":
            messagebox.showwarning("Validation", "Select Status", parent=self.window)
            return False
        return True
    def getName(self):
        try:
